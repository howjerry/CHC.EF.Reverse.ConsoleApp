    ...
    public class EntityGenerator
    {
            ...
            RelationshipType relationship)
        {
            ...
            try
            {
                ...
                var navigationProperty = targetClassName;

                sb.AppendLine($"            // 設定與 {targetClassName} 的一對一關聯");

                if (isSource)
                {
                    // 設定來源端的關聯
                    ConfigureSourceEndOneToOne(sb, foreignKey, targetClassName, navigationProperty);
                }
                else
                {
                    // 設定目標端的關聯
            ...
            string navigationProperty)
        {
            try
            {
                ...
                {
                    sb.AppendLine($"            HasOptional(t => t.{navigationProperty})");
                }
                else
                {
                ...
                // 設定反向導航屬性和外鍵對應
                sb.AppendLine("                .WithOptionalPrincipal()");
                sb.AppendLine($"                .HasForeignKey(t => t.{foreignKeyProperty})");
