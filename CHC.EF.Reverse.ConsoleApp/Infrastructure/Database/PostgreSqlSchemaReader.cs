...
namespace CHC.EF.Reverse.ConsoleApp.Infrastructure.Database
{
    ...
    public class PostgreSqlSchemaReader : IDatabaseSchemaReader
    {
        ...
        private void ReadColumns(NpgsqlConnection conn, TableDefinition table)
        {
            using (var cmd = new NpgsqlCommand(@"
                SELECT 
        ...
        /// <param name="conn">資料庫連線</param>
        /// <param name="table">要讀取的表格定義</param>
        private void ReadPrimaryKey(NpgsqlConnection conn, TableDefinition table)
        {
            using (var cmd = new NpgsqlCommand(@"
        ...
        /// <param name="conn">資料庫連線</param>
        /// <param name="table">要讀取的表格定義</param>
        private void ReadForeignKeys(NpgsqlConnection conn, TableDefinition table)
        {
            using (var cmd = new NpgsqlCommand(@"
                SELECT
        ...
        /// <param name="conn">資料庫連線</param>
        /// <param name="table">要讀取的表格定義</param>
        private void ReadIndexes(NpgsqlConnection conn, TableDefinition table)
        {
            using (var cmd = new NpgsqlCommand(@"
                SELECT
